...
class ScreenRecording extends React.Component {
   ...
   componentDidUpdate(prevProps){
      if(this.props.recordModalVisible && (prevProps.recordModalVisible !== this.props.recordModalVisible)){
      ...
      //console.log("Remote streams var mı ");
   ...
   async invokeGetDisplayMedia(){
      ...
      if(!this.props.micStatus){
         //console.log("mic ", this.props.micStatus );

         notification.warning({
            message: `Notification`,
            description:
            'Your microphone needs to be open!',
            placement: "topRight",
            duration: 2.5,
      ...
      if(this.props.is_electron){
         try {
            const sources = await this.props.electron.desktopCapturer.getSources({ types: ['window', 'screen'] });
         ...
         try{
            ...
            //console.log("1-screen mi track mi ne bu? ");
   ...
   captureScreen(screen){
      try {
         ...
         //console.log("2-burada screen?, ", screen);
   ...
   async setScreens() {
      await this.desktopSources.map(async (source, index) => {
         ...
         if(stream.name === 'Helloooo'){
            ...
            //console.log("setScreen", stream.getAudioTracks());
   ...
   @action.bound
   async stop(){
      //console.log("in stop function ", this.recorder);
   ...
   async stopRecordingCallback(){
      ...
      if (this.recorder.getBlob()) {
         ...
         //console.log("recordPreview ", this.recordPreview);
   ...
   downloadScreenRecordVideo(){
      //console.log("geliyor", this.recordPreview);
      ...
      if (recorderBlob) {
         ...
         var file = new File([blob], 'ScreenRecord' + this.getRandomString() + '.webm', {
            type: 'video/webm'
         });
